    ...
    with open(filename) as csvfile:
        ...
        for line in reader:
            if len(line) == 0:
                ...
                if n not in nkToVariances.keys():
                    nkToVariances[n] = {}
                if k not in nkToVariances[n]:
                    nkToVariances[n][k] = 0
                nkToVariances[n][k]+=float(line[-1])
            elif line[0] == "Count:":
                if n not in nkToCounts.keys():
                    nkToCounts[n] = {}
                if k not in nkToCounts[n]:
                    nkToCounts[n][k] = 0
                nkToCounts[n][k]+=float(line[-1])
    return nkToFinalFitnesses,nkToVariances,nkToCounts
        

landscapes = ["K6", "EO K6", "Random K6", "XOR K6"]
width = .8
fig, ax = plt.subplots()
for i in range(len(landscapes)):
    landscape = landscapes[i]
    nkToFinalFitnesses,nkToVariances,nkToCounts = getnkToFinalFitness(askopenfilename(title = landscape))
    labels = []
    data = []
    error = []
    for n in nkToFinalFitnesses.keys():
        for k in nkToFinalFitnesses[n].keys():
            ...
            error.append(np.sqrt(nkToVariances[n][k])/np.sqrt(nkToCounts[n][k]))
